...
- Requiere `.env` (no versionado): `PORT`, `MONGODB_URI`, `SESSION_SECRET`, `SUPERADMIN_EMAIL`, `SUPERADMIN_PASSWORD`, `GOOGLE_CREDENTIALS_PATH` (opcional, ruta a un archivo local) o `GOOGLE_CREDENTIALS_JSON` (opcional, el JSON completo como texto — para entornos con disco efímero como Render), `ALLOWED_ORIGINS` (opcional), `CLOUDINARY_URL` (opcional — sin esto, fotos/documentos/logos se guardan en disco local; ver sección "Storage de archivos"), `LOGTAIL_SOURCE_TOKEN` + `LOGTAIL_ENDPOINT` (opcionales, solo aplican en producción — sin esto los logs solo salen por stdout, ver sección "Logs y monitoreo"), `SMTP_HOST` + `SMTP_PORT` + `SMTP_USER` + `SMTP_PASS` + `SMTP_FROM` (opcionales, sin esto el envío de formatos por correo desde "Formatos para inspección" responde 503 en vez de fallar feo), `BACKUP_TOKEN` (opcional, sin esto `GET /api/backup` responde 503 — ver sección "Backups").
...
- **Deps:** bcryptjs, compression, cors, dotenv, exceljs, express, express-rate-limit, express-session, googleapis, helmet, mongoose, multer, nodemailer, pdfkit. Dev: nodemon.
...
- `routes/` — auth, superadmin, formatos, admin, registros, empleados, asistencia, **documentos**, **backup** (sin sesión, token fijo — ver sección "Backups").
...
- **Rendimiento al guardar (`POST /api/registros`):** Excel y Google Sheets arrancan los dos al mismo tiempo, pero la respuesta al usuario **solo espera a Excel** (rápido, disco local). **Google Sheets corre de fondo, sin bloquear** — le hace 5-6 llamadas seguidas a su API (chequear/crear pestaña, limpiar, escribir, formatear), eso solo suma 1-2s, y no es algo que el empleado pueda resolver en el momento de todas formas (suele ser permisos o configuración). Si falla, queda logueado (`logger.warn`, llega a Better Stack) pero **ya no se le avisa al usuario en la respuesta** — `googleSheetsError` en la respuesta de `POST /api/registros` siempre es `null` a propósito (medido 2026-09-21: `verificar-atrasado` ~3s → ~870ms con el fix de bcrypt; `registros` seguía en ~2s hasta este cambio, dominado por Sheets). Excel sí se sigue esperando y mostrando su error al instante (ej. "archivo abierto en tu compu"), porque eso sí lo puede arreglar el usuario ahí mismo. El `info` (pendientes/siguienteDía/completoHoy) que se devuelve se arma a mano con el día recién guardado en vez de volver a consultar la BD (verificado que da exactamente lo mismo que una consulta fresca).
...
- **Rendimiento al cargar página:** en `public/js/superadmin-dashboard.js` y `registro-asistencia.js`, las llamadas al API que no dependen entre sí van en `Promise.all` en vez de una detrás de otra — mismo criterio que el guardado de formatos. `cargarEmpresas` sigue esperando a `cargarCatalogo` (necesita `catalogoFormatos` ya cargado), y `cargarResumenEmpleados` sigue esperando a `cargarMeses` (necesita el mes seleccionado) — esas dos dependencias reales no se tocaron, solo se paralelizó lo que sí era independiente.
...
- **Formatos para inspección de sanidad:** botón "🩺 Ver formatos para inspección" en `formatos.html`, junto al de reporte — **los dos botones solo se ven en la vista de las 3 carpetas (Cocina/Salón/Administración), no dentro de una carpeta ya abierta** (ajustado 2026-09-24: `.fila-acciones-formatos` se esconde en `mostrarFormatos()` y se vuelve a mostrar en `volverACarpetas()`, `public/js/formatos.js` — antes se veían tambien dentro de cada carpeta, lo cual no tenia sentido porque el reporte y la inspeccion son de TODAS las carpetas juntas, no de una sola). Mismo patrón de marcador de sesión (`adminInspeccion` en `routes/admin.js`, `POST /api/admin/verificar-inspeccion`, se limpia junto con los demás en `/api/admin/limpiar`). A diferencia del reporte (que muestra % de cumplimiento), esta vista muestra los **datos reales día a día** de todos los formatos activos de un mes elegido, agrupados por carpeta — para tener algo que mostrarle a sanidad en una inspección sin abrir el Excel completo. Backend: `GET /api/registros/inspeccion?anio=&mes=` en `routes/registros.js`, arma las tablas reusando `columnasYFila()` de `servicios/excel.js` (el mismo adaptador que ya convierte cada formato a filas para Excel — se le agregó a `module.exports`, cero lógica de parseo nueva). Un formato sin registros ese mes simplemente no aparece.
  - **Formatos por carpeta arrancan colapsados** en pantalla (botón con nombre, se despliega al hacer clic) — más compacto para no desplegar todo de una. Al imprimir (`window.print()`) se fuerzan todos abiertos (`@media print` en `styles.css`).
  - **Impresión en horizontal:** `#hoja-inspeccion` tiene su propia `@page` con nombre (`page: inspeccion` + `@page inspeccion { size: A4 landscape; margin: 12mm; }`, solo dentro de `@media print`) — estas tablas llegan a tener hasta 15 columnas y en vertical el texto se envolvía demasiado. Cada formato arranca en hoja nueva (`break-before: page` en `.bloque-carpeta` y entre formatos hermanos de una misma carpeta) y la tabla usa letra/padding compactos solo al imprimir. **Ojo, límite real medido (no es un bug, es el texto):** formatos con columnas angostas y valores cortos (ej. Control de Temperatura, Manejo de Residuos) entran en 1 sola hoja incluso con 31 días. Formatos con texto libre largo por día (observaciones largas, la lista de áreas+producto de Limpieza) o muchas filas por día (Presentación Personal con varios empleados) **pueden pasarse a una 2da hoja** aun en horizontal — ahí el cuello de botella es el largo del texto, no el ancho de la tabla, y no hay vuelta de hoja sin sacrificar legibilidad.
  - También se puede **enviar por correo** (`POST /api/registros/inspeccion/enviar`, mismo gate de admin) para cuando sanidad necesita una copia y no está físicamente ahí — armado en `servicios/correo.js` con `nodemailer`. El correo lleva **dos cosas**: el HTML en el cuerpo (para quien solo quiere verlo, con la **misma tabla compacta y los mismos íconos de carpeta que la impresión** — a propósito, porque el correo es justamente para que alguien lo imprima, no solo para mirarlo en pantalla) y un **PDF adjunto** (`servicios/pdfInspeccion.js`, con `pdfkit`) para que sea fácil de imprimir de una sin depender de que el cliente de correo del destinatario respete el html. El PDF arma sus propias tablas desde cero con `pdfkit` (no reusa el html) — A4 horizontal, un formato por sección, columnas de ancho parejo, alto de fila calculado según el texto más largo de esa fila (`doc.heightOfString`) para que nada se corte, salto de página automático si una tabla no cabe entera. Igual que Excel/Sheets, esto **no bloquea la respuesta al usuario más de lo necesario** — se genera en memoria (sin tocar disco) justo antes de mandar el correo, no en `POST /api/registros`, así que no reintroduce la latencia que se optimizó ahí. Igual que Cloudinary/Google Sheets, el envío es **opcional por variables de entorno** (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`) — sin `SMTP_HOST` configurado, el botón "Enviar" responde `503` con un mensaje claro en vez de romper nada.
- **Corregir un registro ya guardado:** cada novedad del reporte de seguimiento tiene un botón "✏️ Corregir" que navega directo a `formato.html?id=&carpeta=&corregir=<registroId>`. **No pide clave aparte** — reusa el mismo marcador que ya dejó activo el reporte (`adminReporteActivo`, `routes/admin.js`): si acabás de generar el reporte, entra directo; el modal de contraseña (que apunta a `/api/admin/verificar-reporte`, el mismo endpoint del reporte) solo aparece si esa sesión ya venció o se entró por URL directa sin pasar por el reporte primero. Backend: `GET /api/registros/registro/:id` trae el registro puntual, `PUT /api/registros/:id` lo actualiza (mismo formato/carpeta/día, solo cambian `responsable`/`observaciones`/`datos`) y resincroniza Excel + Google Sheets igual que un guardado normal — **sin bitácora de auditoría**, se sobreescribe directo. En `formato.js`, el modo corrección reusa el mismo mecanismo que ya existía para pintar "hoy ya guardado" (repuebla el formulario correcto según el tipo de formato, incluyendo recepción con varios ítems y presentación personal con la lista de empleados), solo que deja los campos habilitados en vez de bloqueados (`habilitarFormularioParaCorregir()`) y el botón Guardar pasa a hacer `PUT` en vez de `POST` (`enviarRegistro()` decide según `modoCorreccion`). El reporte también agrega `id` y `carpeta` a cada novedad (antes no los mandaba) para poder armar ese link. **Ojo:** en modo corrección se salta a propósito el gate de "estar dentro de la carpeta Administración" (`esCarpetaAdmin`) — corregir un formato asignado a esa carpeta no debe exigir además haber entrado ahí, porque eso sacaba al admin de la página antes de llegar a corregir nada (bug real, encontrado con presentación personal en una carpeta Administración). Por el mismo motivo, `requireAccesoFormato3` en `routes/empleados.js` (el gate de `GET/POST/PUT/DELETE /api/empleados` cuando presentación personal vive solo en Administración) también acepta `adminReporteActivo` además de `adminCarpetaAdministracionActivo` — si no, `cargarEmpleados()` fallaba con 401 al repoblar la lista de empleados en modo corrección.
...
- **Ajuste manual de horas extra (`models/AjusteHorasExtra.js`):** el administrador puede corregir a mano cualquiera de las 4 categorías del banco de horas de un empleado (`POST /api/asistencia/ajustar-horas-extra`, verificación de contraseña por-request igual que `corregir-salida` — no usa el patrón de marcador de sesión). El nuevo valor **reemplaza** directamente el de esa categoría en `SaldoHorasExtra` (las semanas nuevas que se cierren después siguen sumando/restando sobre ese número, igual que si hubiera salido del cálculo automático). Cada ajuste deja una fila en `AjusteHorasExtra` (empleado, categoría, valor anterior, valor nuevo, motivo opcional, quién y cuándo) puramente informativa — no participa en ningún cálculo. En `registro-asistencia.html`, cada categoría del banco de horas tiene un botón ✏️ que abre el modal de ajuste; debajo del banco hay un "Historial de ajustes" **siempre visible** (no oculto aunque esté vacío) para que el empleado pueda ver en cualquier momento si le tocaron las horas.
...
**Logos en `public/img/`:** `logo-fooddata.png` (lockup con lema — login), `logo-solo.fooddata.png` (solo wordmark — cabeceras). Logos de empresas (subidos) en `public/img/logos/`. Los PNG de marca están comprimidos con paleta de colores (2026-09-22, `sharp` usado una sola vez, no quedó como dependencia) — de ~726KB en total a ~176KB, sin diferencia visible. Si se reemplaza alguno a mano después, conviene comprimirlo antes de subirlo (cualquier optimizador de PNG con paleta sirve).

**Íconos del PWA:** el ícono original (`icon-fooddata.png`) no era cuadrado (621×576) ni transparente, lo que hacía que se viera distinto en celular que en PC al instalarlo. Se arregló en varias pasadas (2026-09-23/24), y terminó en **dos familias de archivos separadas a propósito** — mismo logo, fondo blanco los dos, pero forma distinta según dónde se usan:
- **De escritorio (PC)** — `icon-fooddata-192.png` / `icon-fooddata.png` (512, `manifest.json` `purpose: "any"`). Cuadrados, fondo blanco sólido. Logo grande, casi sin margen (ajustado 2026-09-25: al principio salía del mismo archivo achicado que el maskable de Android, pero Windows no le aplica un recorte agresivo como Android, así que el margen de seguridad no hacía falta ahí y solo lo hacía verse más chico de lo necesario — ahora sale del mismo archivo que el de iOS).
- **De Android (`purpose: "maskable"`)** — `icon-fooddata-maskable.png` (512). Logo achicado con margen, a propósito: Android le aplica su propio recorte adaptativo (circular o redondeado según el launcher) y si el contenido no tiene margen de seguridad, ese recorte le corta el escudo. Es el único de los cuatro que sigue con margen grande.
- **De iOS (`apple-touch-icon`)** — `icon-fooddata-apple.png` (512). Logo grande, casi sin margen, porque iOS no necesita margen de seguridad (su recorte es mucho más suave). Fondo blanco sólido igual que los demás (iOS viejo pinta negro detrás de los píxeles transparentes). El de escritorio (arriba) ahora sale de este mismo archivo.
- **De navegador / pestaña** — `icon-fooddata-favicon.png` (usado en `<link rel="icon">` de cada página) y `public/favicon.ico`. Circulares (recortados con máscara), fondo blanco sólido.
- Los cuatro son variaciones del mismo logo con fondo blanco sólido — la diferencia es solo cuánto margen interno lleva cada uno y qué forma de recorte final le va a aplicar el sistema operativo.
- `public/favicon.ico`: el diálogo de "Instalar esta página como aplicación" de Chrome/Edge en PC no siempre lee el manifest; en varios casos busca directo `/favicon.ico` en la raíz del sitio, y al no existir caía a un avatar genérico con una letra. Armado a mano (sin librería) con 3 tamaños (16/32/48) embebidos como PNG dentro del `.ico` — formato moderno soportado desde Windows Vista, no hacía falta ninguna dependencia nueva. Sale del circular (misma familia que el favicon de pestaña).
...
## Backups
Atlas está en el tier gratis (M0), que **no tiene backups automáticos nativos** (eso empieza en M10+, de pago).
- `node scripts/backup.js` — vuelca cada colección de Mongo a JSON (gzip) + copia `datos/` y `public/img/logos/` completos a `backups/<fecha>/` (esto último solo aplica corriéndolo local — ver abajo). Conserva los últimos 5 backups y borra los más viejos solo.
...
- **Automatizado en producción (2026-09-23):** `.github/workflows/backup.yml` corre todos los días a las 8am UTC (3am Colombia) — también se puede disparar a mano desde la pestaña Actions de GitHub ("Run workflow"). En vez de conectarse directo a Atlas desde GitHub (Atlas solo acepta las IPs de Render, y las de GitHub Actions cambian todo el tiempo — probamos whitelistear dinámicamente con la API de Atlas y no se pudo: Atlas no deja poner `0.0.0.0/0` en la whitelist de una API key), el workflow simplemente le pide el backup **al propio servidor de producción** por HTTPS: `GET /api/backup` (`routes/backup.js`), protegido con un token fijo (`BACKUP_TOKEN`, comparación con `crypto.timingSafeEqual` — no es sesión, es un script llamando, no un navegador) en vez de sesión de superadmin. Como Render ya está en la whitelist de Atlas, no hace falta abrir nada nuevo. El resultado es un único JSON con todas las colecciones (`{ Empresa: [...], Registro: [...], ... }`), que queda como *artifact* de esa corrida (descargable desde GitHub, 90 días de retención). Requiere el secret `BACKUP_TOKEN` en GitHub (mismo valor que la variable de entorno `BACKUP_TOKEN` en Render). Este backup automático **solo cubre MongoDB** — intencional y suficiente: los Excel se reconstruyen enteros desde `Registro` en cada guardado (son 100% regenerables, no son la fuente de verdad), y en producción las fotos/documentos/logos ya viven en Cloudinary, no en el disco de Render.
- El respaldo manual local (`node scripts/backup.js` corrido en esta máquina) sigue sirviendo igual que antes para copiar `datos/`/logos de un dev local con storage en disco — sin `CLOUDINARY_URL`, esas carpetas sí existen y sí se copian. `backups/` ya está en `.gitignore` (contiene datos reales de clientes).
...
- [x] Hecho (2026-09-17): `logger.js` manda los logs también a **Better Stack** en paralelo (además de stdout), usando `@logtail/pino` — solo si `LOGTAIL_SOURCE_TOKEN`/`LOGTAIL_ENDPOINT` existen (si faltan, sigue igual que antes, solo JSON por stdout). Verificado en vivo (logs de arranque del deploy llegando a la "Cola viva" de la fuente "FoodData").
- [x] Hecho (2026-09-17): alerta configurada en Better Stack (Telemetría → Alertas) — dispara por correo cuando aparece 1+ log con `level >= 50` (error) en el servicio FoodData. También hay un monitor de uptime aparte (Better Stack → Uptime) sobre `https://fooddata-vo3e.onrender.com`, creado de paso al configurar todo esto — **ojo:** la app después pasó a tener dominio propio (`fooddata.me`, ver "Pendiente / roadmap"); no se confirmó si el monitor se actualizó para vigilar esa URL en vez de (o además de) la de Render.
...
- **Despliegue: YA HECHO (2026-09-16/17).** La app está en producción en Render, con dominio propio: **`https://fooddata.me`** (conseguido gratis vía GitHub Student Developer Pack + Namecheap; la URL original de Render, `fooddata-vo3e.onrender.com`, sigue existiendo por debajo pero ya no es la que se usa). Todo el checklist obligatorio de la sección "⚠️ ANTES DE DESPLEGAR" está completo y verificado (sesiones persistentes con `connect-mongo`, storage en Cloudinary, credenciales de Google por `GOOGLE_CREDENTIALS_JSON`, Atlas restringido a las IPs de Render, logger con `pino`, `npm audit` limpio). Repaso completo del proyecto hecho en vivo contra una empresa de prueba (superadmin, los 9 formatos, asistencia, Google Sheets, reporte) — todo funcionando. Logs conectados a Better Stack (2026-09-17, `LOGTAIL_SOURCE_TOKEN`/`LOGTAIL_ENDPOINT` en Render), con alerta por correo configurada para errores y un monitor de uptime (ver nota en "Logs y monitoreo" — quedó apuntando a la URL vieja de Render, sin confirmar si se migró al dominio nuevo). Envío de formatos por correo (Resend) también verificado en producción con el dominio `fooddata.me` (DKIM/FPS/DMARC). Queda pendiente, sin urgencia: usuario de DB específico de producción en Atlas (hoy comparte el mismo de desarrollo).
- **Automatizar `backup.js`: HECHO (2026-09-23).** Corre solo todos los días vía GitHub Actions (`.github/workflows/backup.yml`) — ver sección "Backups" para el detalle completo.
