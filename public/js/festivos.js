// Festivos de Colombia — versión frontend (idéntica lógica al módulo del servidor)
(function (global) {
  const cache = new Map();
  ...
  function festivosDeAnio(anio) {
    ...
    const jueves    = suma(pascua, -3);
    const viernes   = suma(pascua, -2);
    const ascension = siguienteLunes(suma(pascua, 40));
    const corpus    = siguienteLunes(suma(pascua, 60));
    const sagrado   = siguienteLunes(suma(pascua, 68));
    ...
    lista.forEach(f => {
      ...
      if (mapa.has(k)) mapa.set(k, mapa.get(k) + ' / ' + f.nombre);
      else mapa.set(k, f.nombre);
    });
    cache.set(anio, mapa);
    return mapa;
  }
  ...
  global.FestivosCO = { esFestivo, nombreFestivo, festivosDeMes, festivosDeAnio };
})(window);
