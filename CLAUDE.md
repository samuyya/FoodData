...
- **De escritorio / app instalada** — `icon-fooddata-192.png` / `icon-fooddata.png` (512, `manifest.json` `purpose: "any"`) e `icon-fooddata-maskable.png` (512, el mismo escudo achicado con margen, para `purpose: "maskable"` de Android y el apple-touch-icon de iOS). **Cuadrados**, fondo blanco sólido — así se ve el ícono en el escritorio de Windows o en la pantalla de inicio del celular al instalar la PWA.
- **De navegador / pestaña** — `icon-fooddata-favicon.png` (usado en `<link rel="icon">` de cada página) y `public/favicon.ico`. **Circulares** (recortados con máscara), fondo blanco sólido — a propósito distinto de los de escritorio (2026-09-24: al principio ambos usos compartían el mismo archivo cuadrado, y eso achataba el ícono redondo que se espera en una pestaña de navegador; ahora son dos archivos separados con la misma marca pero forma distinta).
- Motivo aparte para el apple-touch-icon: iOS viejo pinta negro detrás de los píxeles transparentes, así que ese SIEMPRE tiene que ser sólido (por eso usa el mismo maskable cuadrado, no el circular).
