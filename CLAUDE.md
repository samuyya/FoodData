...
- **Rendimiento al guardar (`POST /api/registros`):** la sincronización de Excel y de Google Sheets **corren en paralelo** (`Promise.all`), no una detrás de la otra — son independientes entre sí. El `info` (pendientes/siguienteDía/completoHoy) que se devuelve en la respuesta se arma a mano con el día recién guardado en vez de volver a consultar la BD (verificado que da exactamente lo mismo que una consulta fresca). Si se agrega otra sincronización externa a este endpoint en el futuro, sumarla también al `Promise.all` en vez de encadenarla con `await` — cada `await` seguido ahí se siente directo en el "Guardar" del usuario.
