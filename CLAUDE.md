...
**Íconos del PWA (`icon-fooddata*.png`):** tres archivos, no uno solo — se detectó (2026-09-23) que el ícono original no era cuadrado (621×576) y no tenía fondo transparente, lo que hacía que el recorte automático de cada plataforma al "instalar"/"agregar a inicio" se viera distinto en celular que en PC. Ahora:
- `icon-fooddata-192.png` / `icon-fooddata.png` (512) — cuadrados, fondo transparente, para `manifest.json` (`purpose: "any"`) y el favicon (`<link rel="icon">` en cada página).
- `icon-fooddata-maskable.png` (512) — el mismo escudo pero achicado con margen sobre fondo blanco sólido, para que el recorte circular/redondeado de Android (`purpose: "maskable"` en `manifest.json`) y el apple-touch-icon de iOS (inyectado por `util.js`, fondo sólido a propósito porque iOS viejo pinta negro detrás de los píxeles transparentes) no corten el diseño.
