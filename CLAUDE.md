...
- **Automatizado en producción (2026-09-23):** `.github/workflows/backup.yml` corre `node scripts/backup.js` todos los días a las 8am UTC (3am Colombia) contra el Atlas real, usando el secret `MONGODB_URI` del repo — también se puede disparar a mano desde la pestaña Actions de GitHub ("Run workflow"). El resultado queda como *artifact* de esa corrida (descargable desde GitHub, 90 días de retención). Este backup automático **solo cubre MongoDB** — el runner de GitHub Actions no tiene acceso al disco de Render, así que `copiarCarpeta()` se salta sola `datos/` y los logos (no da error, simplemente no encuentra esas carpetas). Esto es intencional y suficiente: los Excel se reconstruyen enteros desde `Registro` en cada guardado (son 100% regenerables, no son la fuente de verdad), y en producción las fotos/documentos/logos ya viven en Cloudinary, no en el disco de Render — lo único irremplazable es lo que hay en Mongo, y eso sí queda respaldado.
