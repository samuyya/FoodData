...
- **De escritorio (PC)** — `icon-fooddata-192.png` / `icon-fooddata.png` (512, `manifest.json` `purpose: "any"`). Cuadrados, fondo blanco sólido, logo achicado con margen — así se ve el acceso directo al instalar la PWA en Windows.
- **De Android (`purpose: "maskable"`)** — `icon-fooddata-maskable.png` (512). Mismo logo achicado con margen que el de escritorio, a propósito: Android le aplica su propio recorte adaptativo (circular o redondeado según el launcher) y si el contenido no tiene margen de seguridad, ese recorte le corta el escudo.
- **De iOS (`apple-touch-icon`)** — `icon-fooddata-apple.png` (512, nuevo 2026-09-25). **Archivo aparte del de Android** — el logo grande, casi sin margen, porque iOS no necesita margen de seguridad (su recorte es mucho más suave) y con margen se veía chico comparado con como se ve en Android. Fondo blanco sólido igual que los demás (iOS viejo pinta negro detrás de los píxeles transparentes).
