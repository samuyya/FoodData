...
**Íconos del PWA:** el ícono original (`icon-fooddata.png`) no era cuadrado (621×576) ni transparente, lo que hacía que se viera distinto en celular que en PC al instalarlo. Se arregló en tres partes (2026-09-23/24):
- `icon-fooddata-192.png` / `icon-fooddata.png` (512) — para `manifest.json` (`purpose: "any"`) y el favicon de la pestaña (`<link rel="icon">` en cada página). `icon-fooddata-maskable.png` (512) — el mismo escudo achicado con margen, para el `purpose: "maskable"` de Android y el apple-touch-icon de iOS.
- **Los tres van sobre fondo blanco sólido, ninguno transparente** (ajustado 2026-09-24: al principio `icon-fooddata`/`-192` quedaron con fondo transparente y solo el maskable con blanco — eso hacía que el ícono al instalar la PWA en PC se viera distinto al de celular. Ahora los tres archivos vienen del mismo maskable, así se ven igual en cualquier lado). Motivo aparte para el apple-touch-icon: iOS viejo pinta negro detrás de los píxeles transparentes, así que ese SIEMPRE tiene que ser sólido.
- `public/favicon.ico` — el diálogo de "Instalar esta página como aplicación" de Chrome/Edge en **PC** no siempre lee el manifest; en varios casos busca directo `/favicon.ico` en la raíz del sitio, y al no existir caía a un avatar genérico con una letra. Armado a mano (sin librería) con 3 tamaños (16/32/48) embebidos como PNG dentro del `.ico` — formato moderno soportado desde Windows Vista, no hacía falta ninguna dependencia nueva. También regenerado con fondo blanco, mismo criterio.
