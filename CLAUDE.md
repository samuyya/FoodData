...
**Íconos del PWA:** el ícono original (`icon-fooddata.png`) no era cuadrado (621×576) ni transparente, lo que hacía que se viera distinto en celular que en PC al instalarlo. Se arregló en dos partes (2026-09-23/24):
- `icon-fooddata-192.png` / `icon-fooddata.png` (512) — cuadrados, fondo transparente, para `manifest.json` (`purpose: "any"`) y el favicon de la pestaña (`<link rel="icon">` en cada página). `icon-fooddata-maskable.png` (512) — el mismo escudo achicado con margen sobre fondo blanco sólido, para el `purpose: "maskable"` de Android y el apple-touch-icon de iOS (que necesita fondo sólido a propósito, porque iOS viejo pinta negro detrás de los píxeles transparentes) — esto solo hacía falta para que se viera bien en **celular**.
- `public/favicon.ico` (nuevo) — el diálogo de "Instalar esta página como aplicación" de Chrome/Edge en **PC** no siempre lee el manifest; en varios casos busca directo `/favicon.ico` en la raíz del sitio, y al no existir caía a un avatar genérico con una letra. Armado a mano (sin librería) con 3 tamaños (16/32/48) embebidos como PNG dentro del `.ico` — formato moderno soportado desde Windows Vista, no hacía falta ninguna dependencia nueva.
